...
def vote():
    if request.cookies.get('voted') == 'true':
        return jsonify({"status": "already_voted"}), 403  # 403 Forbidden if already voted
    ...
    response.set_cookie('voted', 'true', max_age=60*60*24)  # Cookie expires in 1 day
    return response
...
def reset_votes():
    ...
    response.set_cookie('voted', '', expires=0)  # Clear the 'voted' cookie for all users
    return response
